# OpenCode Conductor Bridge (v1.1.8)
...
```bash
# Windows/Node
$env:BRIDGE_REF="v1.1.8"; npx github:bardusco/opencode-conductor-bridge

# macOS/Linux
BRIDGE_REF=v1.1.8 curl -sSL https://raw.githubusercontent.com/bardusco/opencode-conductor-bridge/main/install.sh | bash
```
...
If you see permission prompts, add the following to your global OpenCode configuration (usually in your home directory's `.opencode/config.toml` or similar):

```yaml
# Example permission setting
permissions:
  external_directory: "ask" # or "allow"
```
...
1. **Supply Chain:** By default, the installer uses the latest stable tag (e.g., `v1.1.8`). Use `BRIDGE_REF` to pin to a specific SHA for production environments.
