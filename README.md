# OpenCode Conductor Bridge (v1.1.12)
...
### Convenience Script (macOS/Linux)
```bash
curl -sSL https://raw.githubusercontent.com/bardusco/opencode-conductor-bridge/main/install.sh | bash
```

> **Note:** This downloads the installer from `main`. For full script pinning, see [Corporate Installation](#corporateair-gapped-installation).

### Version Pinning
Pin a specific version with `BRIDGE_REF`:
```bash
# Node (recommended for corporate)
BRIDGE_REF=v1.1.12 npx github:bardusco/opencode-conductor-bridge

# Bash (pins content, not the script itself)
BRIDGE_REF=v1.1.12 curl -sSL https://raw.githubusercontent.com/bardusco/opencode-conductor-bridge/main/install.sh | bash

# Full script pinning (bash)
BRIDGE_REF=v1.1.12 curl -sSL https://raw.githubusercontent.com/bardusco/opencode-conductor-bridge/v1.1.12/install.sh | bash
```
...
1. **Supply Chain:** By default, the installer uses the latest stable tag (e.g., `v1.1.12`). Use `BRIDGE_REF` to pin to a specific SHA for production environments.
...
For environments that block `curl | bash` or `npx github:`, use manual installation:
