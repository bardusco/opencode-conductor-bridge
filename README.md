# OpenCode Conductor Bridge (v1.1.11)
...
```bash
# Windows/Node
$env:BRIDGE_REF="v1.1.11"; npx github:bardusco/opencode-conductor-bridge

# macOS/Linux
BRIDGE_REF=v1.1.11 curl -sSL https://raw.githubusercontent.com/bardusco/opencode-conductor-bridge/main/install.sh | bash
```
...
1. **Supply Chain:** By default, the installer uses the latest stable tag (e.g., `v1.1.11`). Use `BRIDGE_REF` to pin to a specific SHA for production environments.
